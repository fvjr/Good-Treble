...
async function writeSongToDatabase(ArtistID, ArtistName, SongID, SongName, userID) {
  console.log(userID);
...
async function getAllPlaylistData(playlist_id){
  const arrayData = await sequelize.query(`SELECT song.name AS SongName, artist.name AS ArtistName FROM playlist
...
async function getArtists(playlist_id){
  const arrayData = await sequelize.query(`SELECT DISTINCT artist.name, COUNT(song.id) AS SONGS FROM artist
